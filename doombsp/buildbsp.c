...
line_t	*CutLine (line_t *wl, divline_t *bl)
{
	...
	*new_p = *wl; 
	...
	intr.x = wld.pt.x + round2(wld.dx*frac);
	intr.y = wld.pt.y + round2(wld.dy*frac);
	offset = wl->offset + round2(frac*sqrt(wld.dx*wld.dx+wld.dy*wld.dy));
...
//float	gray = NX_WHITE; // TODO: nodebuilder draw
...
bspnode_t * BSPList(Array * lines_i)
{
	...
	if (draw)
    {
        SDL_SetRenderDrawColor(nbRenderer, gray*255, gray*255, gray*255, 255);
        gray = 1.0f - gray;
        SDL_SetRenderTarget(nbRenderer, nbTexture);
    }
//		PSsetgray (gray);
//	gray = 1.0 - gray; // TODO: nodebuilder draw
...
void BuildBSP(void)
{
    ...
    if ( draw )
    {
        ...
        for ( int i = 0; i < segstore_i->count; i++ )
        {
            line_t * seg = Get(segstore_i, i);
            SDL_SetRenderDrawColor(nbRenderer, rand()%256, rand()%256, rand()%256, 255);
            NB_DrawLine(seg->p1.x, seg->p1.y, seg->p2.x, seg->p2.y);
        }
        NB_Refresh(10000);
    }
}
